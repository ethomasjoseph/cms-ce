        ...
        extends HandlerInterceptorAdapter
{
        ...
        throws Exception
    {
        ...
        createResponseHeaders( response, traceInfoEncoded );
    }

    // TODO: Make code more testable?
    private void createResponseHeaders( HttpServletResponse response, String traceInfo )
    {
        int pageTraceInfoCharLength = traceInfo.length();

        Integer charsPrHeader = 1000;

        double numberOfHeadersAsDouble = Math.ceil( pageTraceInfoCharLength / charsPrHeader.doubleValue() );
        ...
        for ( int i = 0; i < numberOfHeaders; i++ )
        {
            ...
            if ( endIndex >= pageTraceInfoCharLength )
                endIndex = beginIndex + ( pageTraceInfoCharLength - beginIndex );
